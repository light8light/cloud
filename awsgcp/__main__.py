...
service_account = gcp.serviceaccount.Account(
    "csye6225",
    account_id="csye6225",
    display_name="csye6225"
    )

service_account_id = service_account.account_id.apply(lambda v: f"Service Account ID: {v}")
...
bucket_object_creator_binding = service_account.email.apply(lambda email: gcp.storage.BucketIAMBinding(
...
# 3. Create a public and a private route table
...
# 4. Create public and private subnets in different availability zones
...
# A8:
# 15. Create a load balancer security group
ports = [80, 443]
...
# A5:
# 6. Create application security group
...
# A6:
# 8. Create a Database security group
...
# A7:
# 12. Create IAM role to attach to ec2 instance
...
all_info = pulumi.Output.all(
    database_name, database_username, database_password, database_hostname, sns_topic_arn
)

def render_user_data(args):
    name, username, password, hostname, topicArn = args
...
# googleServiceAccountKey = pulumi.output(service_account_key.private_key)
googleServiceAccountKey = pulumi.Output.all(service_account_key.private_key).apply(lambda args: args[0])
current_file_path = os.path.dirname(os.path.abspath(__file__))
